    ...
    <div>
      <div className="flex items-start justify-between mb-2">
        <div>
          ...
          <p className="text-sm text-ink-3 mt-1">Create projects and load source files. Translators only see the editor.</p>
        </div>
        {!creating && (
          <button
      ...
      {creating && (
        <div className="mt-4 flex gap-2 items-center bg-surface border border-warm-border rounded-[12px] p-3">
          ...
          />
          <button onClick={createProject} disabled={busy} className="px-3 py-2 text-sm font-medium bg-ink text-white rounded-[10px] disabled:opacity-50">
            Create
          </button>
          <button
            onClick={() => {
      ...
      <div className="mt-6 space-y-3">
        {projects.length === 0 && <p className="text-sm text-ink-3">No projects yet.</p>}
        {projects.map((proj) => (
          <div
            key={proj.id}
            className="flex items-center justify-between bg-surface border border-warm-border rounded-[16px] p-5 group"
          >
            {renamingId === proj.id ? (
              ...
              <>
                <Link href={`/translation/admin/${proj.id}`} className="flex-1">
                  ...
                  <p className="text-sm text-ink-3 mt-0.5">Manage sources →</p>
                </Link>
