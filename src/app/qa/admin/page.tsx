...
export default async function QAAdminPage() {
  ...
  return (
    <main className="max-w-4xl mx-auto px-6 py-8">
      <div className="mb-2">
        <Link href="/admin" className="text-xs text-ink-muted hover:text-ink transition-colors">← Admin</Link>
      </div>
      ...
      {projects.length === 0 ? (
        ...
        <div className="space-y-3">
          {projects.map((p) => (
            <div key={p.id} className="bg-surface border border-line rounded-md p-5">
              <div className="flex items-start justify-between gap-4">
                <div className="flex-1 min-w-0">
                  ...
                  {p.description && (
                    <p className="text-sm text-ink-soft mt-0.5 line-clamp-1">{p.description}</p>
                  )}
                ...
                <div className="flex items-center gap-2 shrink-0 flex-wrap justify-end">
                  <Link
                    ...
                    className="px-3 py-1.5 text-xs font-medium bg-canvas border border-line text-ink rounded-xs hover:border-ink transition-colors"
                  >
                    Test suite
                  </Link>
                  <Link
                  ...
                  <Link
                    ...
                    className="px-3 py-1.5 text-xs font-semibold bg-ink text-white rounded-xs hover:opacity-90 transition-opacity"
                  >
                    Dashboard
                  </Link>
                </div>
