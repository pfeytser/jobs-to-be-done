  ...
  return (
    <main className="max-w-wide mx-auto px-6 py-8">
      ...
      <div className="mb-6 flex items-center gap-2 flex-wrap">
        <Link href="/qa/admin" className="text-xs text-ink-muted hover:text-ink transition-colors">← Admin</Link>
        <span className="text-ink-muted text-xs">/</span>
        <span className="text-xs text-ink-soft">{project.name}</span>
      </div>
      ...
      <div className="mb-6 flex items-start justify-between gap-4 flex-wrap">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          <Link
            href={`/qa/${projectId}`}
            className="px-3 py-2 text-sm font-medium bg-canvas border border-line text-ink rounded-xs hover:border-ink transition-colors"
          >
            Start session
          </Link>
          <Link
          ...
          <a
            ...
            className="px-3 py-2 text-sm font-semibold bg-ink text-white rounded-xs hover:opacity-90 transition-opacity"
          >
            Export ZIP
          </a>
        </div>
      ...
      {/* Status control */}
      <div className="mb-6 p-4 bg-surface border border-line rounded-md flex items-center gap-3 flex-wrap">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-8">
        {[
          ...
          <div key={card.label} className="bg-surface border border-line rounded-md p-4">
      ...
      <section className="mb-8">
        ...
        {project.user_types.length === 0 ? (
          <Link
            ...
            className="flex items-center justify-between p-4 bg-surface border border-line rounded-md hover:border-ink transition-colors group"
          >
            <div>
              <p className="text-sm font-medium text-ink">All test items</p>
              <p className="text-xs text-ink-muted mt-0.5">{items.length} item{items.length !== 1 ? 's' : ''}</p>
            </div>
            <svg className="w-4 h-4 text-ink-muted group-hover:text-ink transition-colors" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              ...
              return (
                ...
                >
                  <div className="min-w-0 flex-1">
                    ...
                    <p className="text-xs text-ink-muted mt-0.5">{count} item{count !== 1 ? 's' : ''}</p>
                  </div>
                  <svg className="w-4 h-4 text-ink-muted group-hover:text-ink transition-colors shrink-0 ml-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5l7 7-7 7" />
                  </svg>
                </Link>
              )
              ...
              <thead>
                <tr className="bg-canvas border-b border-line">
                  {['Tester', 'User type', 'Viewport / OS / Browser', 'Progress', 'Pass', 'Fail', 'Blocked', 'Last active', 'Status', ''].map((h) => (
                    <th key={h} className="px-3 py-2 text-left text-xs font-semibold text-ink-muted whitespace-nowrap">{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                  ...
                  return (
                    <tr key={s.id} className="border-b border-line last:border-0 hover:bg-canvas">
                      <td className="px-3 py-2.5">
                        ...
                        <p className="text-xs text-ink-muted">{s.tester_email}</p>
                      </td>
                      <td className="px-3 py-2.5 text-sm text-ink-soft">{s.user_type}</td>
                      <td className="px-3 py-2.5">
                        <p className="text-xs text-ink-soft">{s.viewport} · {s.operating_system} · {s.browser}</p>
                      </td>
                      <td className="px-3 py-2.5">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-xs text-ink-soft whitespace-nowrap">{s.done}/{s.total} ({pct}%)</span>
                        </div>
                      </td>
                      <td className="px-3 py-2.5 text-center text-xs text-pass font-medium">{s.passed}</td>
                      <td className="px-3 py-2.5 text-center text-xs text-fail font-medium">{s.failed}</td>
                      <td className="px-3 py-2.5 text-center text-xs text-blocked font-medium">{s.blocked}</td>
