  ...
  return (
    <main className="max-w-content mx-auto px-6 py-10 space-y-10">
      ...
      <section>
        ...
        <p className="text-sm text-ink-muted mb-5">Who is the customer in this storyboard?</p>

        <div className="space-y-4 p-5 bg-surface border border-line rounded-md">
          <Field
            label="Customer name"
      ...
      {profileSaved && (
        <section>
          <div className="mb-5">
            ...
            <p className="text-sm text-ink-muted mt-0.5">Add scenes in chronological order.</p>
          </div>

          {cards.length === 0 ? (
            <div className="text-sm text-ink-muted py-10 text-center border border-dashed border-line rounded-md">
          ...
          <button
            onClick={addCard}
            disabled={addingCard}
            className="mt-4 w-full py-2.5 border-2 border-dashed border-line text-sm font-medium text-ink-muted rounded-md hover:border-ink hover:text-ink transition-colors disabled:opacity-40"
          >
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-1">
          <button
            onClick={() => onMove('up')}
            disabled={index === 0}
            className="p-1 text-ink-muted hover:text-ink transition-colors disabled:opacity-30"
            title="Move up"
          >
