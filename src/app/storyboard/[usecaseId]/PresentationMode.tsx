  ...
  if (cards.length === 0) {
    return (
      <main className="flex items-center justify-center min-h-[60vh] px-6">
        <p className="text-ink-muted text-sm">No scenes have been added to this storyboard yet.</p>
      </main>
    )
  }
  ...
  return (
    <main className="min-h-[calc(100vh-57px)] flex flex-col">
      ...
      <div className="flex-1 flex flex-col items-center px-6 pb-6 max-w-4xl mx-auto w-full">
        {/* Image area */}
        <div className="w-full aspect-video bg-canvas border border-line rounded-lg overflow-hidden mb-6 relative">
          <ImageSlot
            card={card}
            index={currentIndex}
            isAdmin={isAdmin}
        ...
        <div className="flex items-center gap-6 mt-8">
          <button
            ...
            disabled={!hasPrev}
            className="flex items-center gap-2 px-5 py-2.5 border border-line rounded-lg text-sm font-medium text-ink hover:border-ink transition-colors disabled:opacity-30 disabled:cursor-not-allowed"
          >
...
function ImageSlot({
  ...
  if (state === 'generating' || retrying) {
    return (
      <div className="w-full h-full flex flex-col items-center justify-center gap-3">
        <svg className="w-6 h-6 text-ink-muted animate-spin" fill="none" viewBox="0 0 24 24">
          <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4" />
          <path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8v8z" />
        </svg>
        <p className="text-sm text-ink-muted">Generating image…</p>
        <p className="text-xs text-ink-muted opacity-60">This takes about 20–30 seconds</p>
      </div>
    )
  }
  ...
  if (state === 'failed') {
    return (
      <div className="w-full h-full flex flex-col items-center justify-center gap-3">
        ...
        <p className="text-sm text-ink-muted">Image generation failed</p>
        {isAdmin && (
          <button
            onClick={handleRetry}
            className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium border border-line rounded-lg text-ink-muted hover:text-ink hover:border-ink transition-colors"
          >
  ...
  return (
    <div className="w-full h-full flex flex-col items-center justify-center gap-3">
      ...
      <p className="text-sm text-ink-muted">Image not yet generated</p>
      {isAdmin && (
        <button
          onClick={handleRetry}
          className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium border border-line rounded-lg text-ink-muted hover:text-ink hover:border-ink transition-colors"
...
function AdminRegenerateButton({
  ...
  return (
    <button
      onClick={handleClick}
      disabled={busy}
      className="flex items-center gap-1.5 px-3 py-1 text-xs font-medium border border-line rounded-lg text-ink-muted hover:text-ink hover:border-ink transition-colors disabled:opacity-40"
    >
      <svg
