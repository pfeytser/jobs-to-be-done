...
export function FailureLog({ initialFailures }: { initialFailures: Failure[] }) {
  ...
  return (
    <div className="space-y-3">
      {failures.map((f) => (
        <div key={f.id} className="bg-surface border border-fail-line rounded-md p-4">
          <div className="flex items-start justify-between gap-3 mb-2">
            ...
            <div className="flex items-center gap-2 shrink-0">
              {f.tc_number && (
                <span className="text-xs font-mono text-ink-muted">{f.tc_number}</span>
              )}
              <button
                onClick={() => handleAcknowledge(f.id)}
                disabled={acknowledging === f.id}
                className="px-2.5 py-1 text-xs font-medium bg-canvas border border-line text-ink-soft rounded-full hover:border-ink hover:text-ink transition-all disabled:opacity-50"
              >
          ...
          {f.steps_taken && (
            <div className="mb-1.5">
              <span className="text-xs font-semibold text-ink-muted">Steps taken: </span>
              <span className="text-xs text-ink-soft">{f.steps_taken}</span>
            </div>
          )}
          {f.actual_behavior && (
            <div className="mb-1.5">
              <span className="text-xs font-semibold text-ink-muted">What happened: </span>
              <span className="text-xs text-ink-soft">{f.actual_behavior}</span>
            </div>
          )}
          {f.test_username && (
            <div className="mb-1.5">
              <span className="text-xs font-semibold text-ink-muted">Username used: </span>
              <span className="text-xs font-mono text-ink-soft">{f.test_username}</span>
            </div>
          )}
              ...
              {f.screenshot_url ? (
                ...
                <a href={`/api/qa/blob-image?url=${encodeURIComponent(f.screenshot_url)}`} target="_blank" rel="noopener noreferrer">
                  <img
                    ...
                    className="max-h-32 rounded-xs border border-line object-cover hover:opacity-80 transition-opacity"
                  />
                </a>
              ) : (
                <span className="text-xs font-mono text-ink-muted">{f.screenshot_filename}</span>
              )}
            </div>
          )}
